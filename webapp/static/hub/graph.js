...
(function bootstrapGephiLite() {
  ...
  function buildGraph() {
    ...
    try {
      ...
      const iterations = isFiltered ? 50 : 300; // Adaptive iterations
      console.time("FA2-Layout");
      window.forceAtlas2.assign(graph, { iterations: iterations, settings: spreadSettings });
  ...
  class Signal {
    ...
    draw(ctx, renderer) {
      ...
      ctx.fill();
      // Optimization: Removed shadowBlur as it's expensive in high-density graphs
    }
  }
  ...
  function initBackgroundFlow() {
    ...
    const count = 150; // Optimized particle count
  ...
  refs.search && (refs.search.oninput = () => {
    if (searchTimeout) clearTimeout(searchTimeout);
    searchTimeout = setTimeout(() => rebuildFromFilters(), 250);
  });
